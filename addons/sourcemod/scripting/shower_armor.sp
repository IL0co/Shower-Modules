...
public void OnPluginStart()
{
	...
	g_hCookie = RegClientCookie("shower_armor", "shower_armor", CookieAccess_Private);
	...
	(cvar_cTimeUpdate = CreateConVar("sm_shower_armor_update_time", "0.5", "Время проверки изменения брони. Требуется рестарт плагина!\nEN: Check time changes armor. Restart plugin required!", _, true, 0.1)).AddChangeHook(CVarChanged);
	...
	AutoExecConfig(true, "shower_armor");
...
public void CreditsCookieHandler(int client, CookieMenuAction action, any info, char[] buffer, int maxlen)
{
	...
	switch (action)
    {
		case CookieMenuAction_DisplayOption:
		{
			...
			FormatEx(buffer, maxlen, "%t%t", g_bHUD[client] ? "Plus" : "Minus", "Armor");
		}
