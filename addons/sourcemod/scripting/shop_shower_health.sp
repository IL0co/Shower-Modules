...
{
	name		= "[Shop] Shower of Health",
	version		= "1.0",
	description	= "Shower of your hp in shop",
	author		= "ღ λŌK0ЌЭŦ ღ ™",
	url			= "https://github.com/IL0co"
}

#define CATEGORY	"shower"
#define ITEM	"shower_healt"
...
public void OnPluginStart()
{
	...
	(cvar_cEnable = CreateConVar("sm_shop_shower_health_enable", "1", "RU: Включен ли плагин?\nEN: Enabled plugin?", _, true, 0.0, true, 1.0)).AddChangeHook(CVarChanged);
	cEnable = cvar_cEnable.BoolValue;

	(cvar_cCount = CreateConVar("sm_shop_shower_health_count", "5", "RU: Количество столбиков\nEN: Number of posts", _, true, 1.0, true, 5.0)).AddChangeHook(CVarChanged);
	cCount = cvar_cCount.IntValue;

	(cvar_cType = CreateConVar("sm_shop_shower_health_type", "2", "RU: На сколько смещать каждую новую строчку? (в пробелах)\nEN: How much to shift each new line? (in spaces)", _, true, 0.0, true, 7.0)).AddChangeHook(CVarChanged);
	cType = cvar_cType.IntValue;

	(cvar_cTypePoss = CreateConVar("sm_shop_shower_health_type_poss", "0", "RU: Куда уходит лесенка начиная от прицела?, 0 - в левый низ, 1 - в правый вверх, 2 - в правый низ, 3 - в левый вверх\nEN: Where does the ladder go from the sight ?, 0 - to the left bottom, 1 - to the right up, 2 - to the right bottom, 3 - to the left up", _, true, 0.0, true, 3.0)).AddChangeHook(CVarChanged);
	cTypePoss = cvar_cTypePoss.IntValue;

	(cvar_cTimeClean = CreateConVar("sm_shop_shower_health_clean_time", "2.0", "RU: Время очищения истории\nEN: History Cleansing Time", _, true, 0.1)).AddChangeHook(CVarChanged);
	cTimeClean = cvar_cTimeClean.FloatValue;

	(cvar_cTimeUpdate = CreateConVar("sm_shop_shower_health_update_time", "0.5", "RU: Время проверки изменения хп. Требуется рестарт плагина!\nEN: Check time changes hp. Restart plugin required!", _, true, 0.1)).AddChangeHook(CVarChanged);
	cTimeUpdate = cvar_cTimeUpdate.FloatValue;

	(cvar_cHudColor = CreateConVar("sm_shop_shower_health_rgba", "255 0 255 255", "RU: RGBA цвет худа\nEN: RGBA color hud")).AddChangeHook(CVarChanged);
	SetHUDColor(cvar_cHudColor);

	(cvar_cHudPoss = CreateConVar("sm_shop_shower_health_poss", "0.38 0.5", "RU: XY позиция худа\nEN: XY hud position")).AddChangeHook(CVarChanged);
	SetHUDPosition(cvar_cHudPoss);

	(cvar_cTimeHide = CreateConVar("sm_shop_shower_health_hide_time", "2.0", "RU: Время скрытия худа\nEN: Hud hiding time", _, true, 0.1)).AddChangeHook(CVarChanged);
	cTimeHide = cvar_cTimeHide.FloatValue;

	(cvar_Price = CreateConVar("sm_shop_shower_health_price", "500", "RU: Цена предмета \nEN: Item price")).AddChangeHook(CVarChanged);
	cPrice = cvar_Price.IntValue;

	(cvar_SellPrice = CreateConVar("sm_shop_shower_health_sellprice", "250", "RU: Цена продажи предмета \nEN: Item selling price")).AddChangeHook(CVarChanged);
	cSellPrice = cvar_SellPrice.IntValue;

	(cvar_Duration = CreateConVar("sm_shop_shower_health_duration", "86400", "RU: Длительность предмета \nEN: Item Duration")).AddChangeHook(CVarChanged);
	cDuration = cvar_Duration.IntValue;

	AutoExecConfig(true, "shop_shower_health", "shop");
...
public void Shop_Started()
{
	...
	{
		Shop_SetInfo("shower_healt", "", cPrice, cSellPrice, Item_Togglable, cDuration);
...
{
	FormatEx(buffer, maxlen, "%T", "shower_healt", client);
	return true;
}
...
{
	FormatEx(buffer, maxlen, "%T", "shower_healt_description", client);
	return true;
}
